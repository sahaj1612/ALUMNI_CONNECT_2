...
export function AdminPortalPage() {
  ...
  async function load(nextSearch = search) {
    ...
    const response = await apiRequest(`/admin/dashboard${query}`, { token });
    setData(response);
  }
  ...
  return <PortalLayout title="Administrator Panel" section={section} onSectionChange={setSection} navigation={navigation}>
    ...
    {section === "dashboard" && <section className="content-stack">
      <Summary items={[["Students", data.summary.studentCount], ["Alumni", data.summary.alumniCount], ["Total Accounts", data.summary.totalAccounts]]} />
      <div className="panel-card"><h3>Find an account</h3><div className="inline-actions"><input value={search} placeholder="Search by name, email, or USN" onChange={(e) => setSearch(e.target.value)} /><button className="primary-button" onClick={() => action(() => load(search), "Account list updated.")}>Search</button><button className="secondary-button" onClick={() => { setSearch(""); action(() => load(""), "Search cleared."); }}>Clear</button></div></div>
    </section>}
...
function Summary({ items }) { return <div className="summary-grid">{items.map(([label, value]) => <article key={label} className="summary-card"><strong>{value}</strong><span>{label}</span></article>)}</div>; }
